...
このリポジトリは Tezuna の**配布専用**です（ソースコードは非公開）。
...
`Tezuna_<version>_x64-setup.exe` をダウンロードして実行。
...
1. PC の Tezuna → 設定 (⚙) → 「スマホ連携」→ **有効にする**
2. 表示された QR コードをスマホのカメラで読み、ブラウザで開く（同一 Wi-Fi 内）
